...
def inference(train, images, visualize=False):
    ...
    fc2_w = tf.Variable(tf.truncated_normal([layer1, layer2], stddev=0.1), name='fc2_w')
    fc2_b = tf.Variable(tf.constant(0.1, shape=[layer2]), name='fc2_b')
    fc3_w = tf.Variable(tf.truncated_normal([layer2, num_labels], stddev=0.1), name='fc3_w')
    fc3_b = tf.Variable(tf.constant(0.1, shape=[num_labels]), name='fc3_B')

    # Model.
    def train_model(data):
        # 3 basic convolution and pooling layers followed by 3 fully connected layers
        ...
        hidden2 = tf.nn.relu(tf.matmul(hidden_dropout, fc2_w) + fc2_b)
        hidden2_dropout = tf.nn.dropout(hidden2, hidden_dprob)
        output = tf.matmul(hidden2_dropout, fc3_w) + fc3_b
        return output
    ...
    def test_model(data):
        ...
        reshape = tf.reshape(pool3, [shape[0], shape[1] * shape[2] * shape[3]])
        hidden = tf.nn.relu(tf.matmul(reshape, fc1_w) + fc1_b)
        hidden2 = tf.nn.relu(tf.matmul(hidden, fc2_w) + fc2_b)
        output = tf.matmul(hidden2, fc3_w) + fc3_b
    ...
    if train:
        ...
        relu1, relu2, relu3 = test_model(images)
    else:
...
def run_training(lr, num_epochs, path, ss_path):
    """
    Run the training for the model
    :param lr: The learning rate
    :param num_epochs: Number of full passes through the data to take
    :param path: Path to save/restore the model
    :param ss_path: Path to save/restore the convolutional layers for semantic segmentation
    """
    with tf.Graph().as_default():
        ...
        if os.path.isfile(ss_path):
            ss_saver.restore(sess=sess, save_path=path)
            print 'Model Restored'
        else:
            sess.run(init_op)
            print 'Model Initialized'
...
def basic_model(lr, num_epochs):
    """
    Run the basic_convnet model
    :param lr: The learning rate
    :param num_epochs: The number of epochs
    """
    checkpoint_file = './base/base.ckpt'
    wd = os.getcwd()
    if not os.path.exists(wd + '/base'):
        os.mkdir(wd + '/base')
